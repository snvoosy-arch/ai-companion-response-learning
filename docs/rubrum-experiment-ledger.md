...
240개 ContentPlan과 1,920개 후보를 사용한 이전 ranker는 dev 허용 top-1 `0.975`, frozen heldout `1.0`을 기록했지만 결정론적 selector를 엄격히 이기지 못했습니다.
...
현재 운영 MeaningBERT-A에서 피로 6건, 배고픔 6건, 심심함 8건의 직접 `meaning_model` frame을 확인했습니다.
...
후보와 최종 출력의 SHA가 일치했고 실제 Discord 전달과 TurnOutcome 최종화가 성공했습니다. 개인 메시지, 사용자·채널·결정 식별자는 공개하지 않습니다.
