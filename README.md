...
식별자, 개인 대화 로그, 토큰과 로컬 DB는 포함하지 않습니다. `white` 경로는 초기
개발 명칭과 실행 호환성을 위해 유지합니다.
...
After
LLM이 행동을 제안함
→ runtime이 capability와 permission을 확인함
→ executor가 허용된 행동만 수행함
→ evidence가 실제 결과를 기록함
→ delivery gate가 최종 발화를 전달함
```
...
Known-failure replay에서 22건을 모두 격리했지만 유용하게 복구한 것은 5건뿐이었던
결과와, V5가 수량 의미를 잘못 보존한 결과는 guard가 Actor의 의미 품질을 대신할 수
없다는 근거입니다.
...
python scripts/check_sapphirus_portfolio.py
```

예제는 Mock Actor가 read-only 도구를 제안하고, 외부 gate가 권한을 검사하고,
fixture executor의 결과를 두 번째 Actor 호출에 전달한 뒤 최종 delivery와 evidence
ledger를 기록합니다.
...
private artifact로 보관하며 사용자 문장과 모델 원문 출력은 공개하지 않습니다.
