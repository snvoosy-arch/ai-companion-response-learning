...
class SharedRuntimeStateStore:
    ...
    def _configure_connection(self, connection: sqlite3.Connection) -> sqlite3.Connection:
        connection.row_factory = sqlite3.Row
        connection.execute(f"PRAGMA busy_timeout = {SQLITE_BUSY_TIMEOUT_MS}")
        try:
            connection.execute("PRAGMA journal_mode = WAL")
        except sqlite3.OperationalError:
            # Cross-OS/shared SQLite paths (for example WSL -> /mnt/c) can reject WAL sporadically.
            # Fall back to the default journal mode instead of failing the whole runtime presence path.
    ...
    def _connect(self) -> sqlite3.Connection:
        ...
        return self._configure_connection(connection)
