...
DEFAULT_SOURCE = ROOT / "data" / "examples" / "broadcast_judgment_examples_128.jsonl"
DEFAULT_MODEL_OUT = ROOT / "models" / "broadcast_action_centroid.json"
DEFAULT_REPORT_OUT = ROOT / "reports" / "broadcast_action_centroid_metrics.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="방송형 판단 시드셋으로 action 예측 베이스라인 모델을 학습합니다."
    )
...
def main() -> None:
    ...
    print("broadcast action model trained")
...
def render_input_text(user_text: str, state: dict) -> str:
    ...
    if state.get("repeat_offense") is not None:
        parts.append(f"repeat={state['repeat_offense']}")
...
def split_rows(rows: list[dict[str, str]], *, eval_ratio: float, seed: int) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    for keys in grouped_by_action.values():
        ...
        if len(keys) <= 1:
            continue
        eval_group_count = int(len(keys) * eval_ratio)
        if eval_group_count <= 0:
            eval_group_count = 1
        if eval_group_count >= len(keys):
            eval_group_count = max(1, len(keys) - 1)
        eval_groups.update(keys[:eval_group_count])
...
def evaluate(model: CharNgramCentroidModel, rows: list[dict[str, str]]) -> dict:
    ...
    return {
        "source_note": "작은 방송형 시드셋으로 action만 먼저 맞히는 베이스라인입니다.",
        "eval_records": total,
