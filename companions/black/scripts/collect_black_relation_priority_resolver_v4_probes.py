...
DATE_STEM = "20260526"
PREFIX = f"black_relation_priority_resolver_v4_practical_residual_probe_collection_{DATE_STEM}"
DEFAULT_REPORTS = (
    REPORT_DIR / "black_relation_priority_resolver_v3_on_v26_eval_20260526.json",
    REPORT_DIR / "black_relation_priority_resolver_v3_on_v27_eval_20260526.json",
)
DEFAULT_OUT_ALL = DATA_DIR / f"{PREFIX}_all.jsonl"
DEFAULT_OUT_SUMMARY = REPORT_DIR / f"{PREFIX}_summary.json"

TARGET_ROLES = {
    "practical_false_negative",
}
CONTROL_ROLES = {
    "practical_boundary_false_positive",
}
WATCH_ROLES = {
    "other_residual_watch",
}
ROLE_RANK = {
    "practical_false_negative": 0,
    "practical_boundary_false_positive": 1,
    "other_residual_watch": 2,
}
NONE_LABEL = "__none__"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Collect v4 probe rows from relation-priority resolver v3 practical residual errors."
    )
...
def probe_role(error: dict[str, Any]) -> str | None:
    ...
    if gold == "practical_first" and resolver == NONE_LABEL:
        return "practical_false_negative"
    if gold == NONE_LABEL and resolver == "practical_first":
        return "practical_boundary_false_positive"
...
def _probe_row(index: int, error: dict[str, Any], *, prefix: str) -> dict[str, Any]:
    ...
    return {
        ...
        "label_status": "relation_priority_resolver_v4_probe",
        "probe_family": family,
        "probe_role": role,
        "targets": {
        ...
        "meta": {
            "source": "black_relation_priority_resolver_v4_probe_collection",
...
def build_summary(
    ...
    *,
    ...
    return {
        ...
        "next_action": "Use target rows for resolver v4 practical recall; keep control rows as gas-stove/status-boundary regression checks.",
    }
        ...
        json.dumps(
            {
                ...
                "control_row_count": summary["control_row_count"],
            },
            ensure_ascii=False,
        )
    )
