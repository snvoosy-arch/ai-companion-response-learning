...
BUILD_SCRIPT = ROOT / "scripts" / "build_black_live30_mixed_repair_20260426.py"
TRAIN_SCRIPT = ROOT / "scripts" / "train_kobart_black.py"
ALL_PATH = ROOT / "data" / "black_live30_mixed_repair_20260426_all.jsonl"
OUTPUT_DIR = WORKSPACE_ROOT / "models" / "candidates" / "black" / "generation" / "kobart_black_live30_mixed_repair_20260426"
REPORT_PATH = ROOT / "reports" / "kobart_black_live30_mixed_repair_20260426_train_report.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Black live30 mixed repair용 KoBART 미세보정을 위한 build + train 런처"
    )
    parser.add_argument("--build-only", action="store_true", help="mixed SFT만 만들고 학습은 건너뜀")
    ...
    parser.add_argument("--eval-ratio", type=float, default=0.12, help="train 스크립트에 넘길 eval 비율")
    ...
    parser.add_argument("--learning-rate", type=float, default=1e-5, help="학습률")
    ...
    parser.add_argument("--live30-weight", type=int, default=3, help="live30 repair row 가중 반복 횟수")
    parser.add_argument(
        "--model-name-or-path",
